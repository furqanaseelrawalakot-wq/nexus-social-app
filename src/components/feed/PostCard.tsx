...
export const PostCard: React.FC<{ post: Post }> = ({ post }) => {
  ...
  const [editPostFeeling, setEditPostFeeling] = useState<string | undefined>(undefined);
  const [editPostLocation, setEditPostLocation] = useState<string | undefined>(undefined);
  const [showEditFeelingPicker, setShowEditFeelingPicker] = useState(false);
  const [showEditLocationInput, setShowEditLocationInput] = useState(false);
  const [isSavingPostEdit, setIsSavingPostEdit] = useState(false);

  // Post Report State
  const [showReportModal, setShowReportModal] = useState(false);
  const [reportReason, setReportReason] = useState('Spam or unwanted promotional content');
  const [reportCustomDetails, setReportCustomDetails] = useState('');
  ...
  const reportReasons = [
    ...
    'Nudity, sexual content, or pornography',
    'Violence, physical harm, or illegal goods',
    'False information, scam, or impersonation',
    'Other issue',
  ];
  ...
  const handleStartEditPost = () => {
    ...
    setShowEditLocationInput(false);
  ...
  const handleSubmitReport = async (e: React.FormEvent) => {
    ...
    const fullReason =
      reportReason === 'Other issue' && reportCustomDetails.trim()
        ? `Other: ${reportCustomDetails.trim()}`
        : reportReason;
    const success = await reportPost(post.id, fullReason);
    setIsSubmittingReport(false);
    if (success) {
      setShowReportModal(false);
      setReportReason('Spam or unwanted promotional content');
      setReportCustomDetails('');
    }
  };
          ...
          <div className="bg-white rounded-3xl max-w-md w-full p-6 shadow-2xl border border-slate-100 space-y-5 animate-in zoom-in-95">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h3 className="text-base font-bold text-slate-900">Report Post</h3>
                  <p className="text-xs text-slate-500">Help us understand what's wrong with this post</p>
                </div>
              </div>
              <button
                type="button"
            ...
            <form onSubmit={handleSubmitReport} className="space-y-4">
              ...
              {reportReason === 'Other issue' && (
                <div className="space-y-1.5 animate-in fade-in">
                  ...
                  <textarea
                    value={reportCustomDetails}
                    onChange={(e) => setReportCustomDetails(e.target.value)}
                    placeholder="Provide additional context about why you are reporting this post..."
                    rows={2}
                    className="w-full px-3.5 py-2 text-xs rounded-2xl border border-slate-200 focus:outline-none focus:border-indigo-500 text-slate-800 resize-none"
                  />
                </div>
              )}
              ...
              <div className="flex items-center justify-end gap-2.5 pt-2">
                ...
                <button
                  ...
                  className="px-5 py-2 rounded-xl text-xs font-bold text-white bg-rose-600 hover:bg-rose-700 disabled:opacity-50 transition-colors shadow-xs flex items-center gap-1.5"
                >
                  {isSubmittingReport ? (
                    <>
      ...
      <article className="rounded-3xl bg-white border border-slate-200 shadow-card hover:shadow-card-hover transition-all duration-300 p-4 sm:p-6 space-y-4 select-none">
        ...
        <div className="flex items-start justify-between">
          ...
          <div className="relative">
            ...
            {showMenu && (
              <div className="absolute right-0 mt-1 w-48 rounded-2xl bg-white border border-slate-200 shadow-xl p-1.5 z-20 animate-in fade-in">
                <button
                  type="button"
                  onClick={() => {
                ...
                <button
                  ...
                  className="w-full flex items-center gap-2 px-3 py-2 rounded-xl text-xs font-semibold text-amber-600 hover:bg-amber-50 transition-colors"
                >
        ...
        {/* EDIT POST COMPOSER */}
        {isEditingPost ? (
          <form onSubmit={handleSavePostEdit} className="p-4 rounded-2xl border-2 border-indigo-200 bg-indigo-50/30 space-y-3 animate-in fade-in">
            <div className="flex items-center justify-between border-b border-indigo-100 pb-2">
              <span className="text-xs font-bold text-indigo-900 flex items-center gap-1.5">
                <Edit2 className="w-3.5 h-3.5" /> Editing Post
              </span>
              <div className="flex items-center gap-1">
                {/* Visibility selector */}
                <div className="flex items-center bg-white border border-indigo-200 rounded-xl p-0.5 text-[11px] font-semibold">
                  <button
                    type="button"
                    onClick={() => setEditPostVisibility('public')}
                  ...
                  <button
                    type="button"
                    onClick={() => setEditPostVisibility('friends')}
                    className={`px-2 py-1 rounded-lg flex items-center gap-1 transition-colors ${
                      editPostVisibility === 'friends'
                        ? 'bg-indigo-600 text-white shadow-xs'
                        : 'text-slate-600 hover:bg-slate-50'
                    }`}
                  >
                  ...
                  <button
                    type="button"
                    onClick={() => setEditPostVisibility('private')}
                    className={`px-2 py-1 rounded-lg flex items-center gap-1 transition-colors ${
                      editPostVisibility === 'private'
                        ? 'bg-indigo-600 text-white shadow-xs'
                        : 'text-slate-600 hover:bg-slate-50'
                    }`}
                  >
            ...
            <textarea
              ...
              rows={3}
              className="w-full px-3.5 py-2.5 text-xs sm:text-sm rounded-xl border border-slate-200 bg-white text-slate-900 focus:outline-none focus:border-indigo-500 resize-none leading-relaxed"
              autoFocus
            />

            {/* Feeling & Location active tags */}
            {(editPostFeeling || editPostLocation) && (
              <div className="flex flex-wrap items-center gap-2 text-xs">
                {editPostFeeling && (
                  <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-amber-100 text-amber-800 font-medium">
                    <span>Feeling {editPostFeeling}</span>
                    <button
                      type="button"
                      onClick={() => setEditPostFeeling(undefined)}
                ...
                {editPostLocation && (
                  <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-emerald-100 text-emerald-800 font-medium">
                    ...
                    <span>{editPostLocation}</span>
                    <button
                      type="button"
                      onClick={() => setEditPostLocation(undefined)}
            ...
            {showEditFeelingPicker && (
              <div className="p-2 rounded-xl bg-white border border-slate-200 flex flex-wrap gap-1.5 animate-in fade-in">
                {feelings.map((f) => (
                  <button
                    key={f.label}
                    type="button"
                    onClick={() => {
                      setEditPostFeeling(f.label);
                      setShowEditFeelingPicker(false);
                    }}
                    className="px-2.5 py-1 rounded-lg bg-slate-50 hover:bg-indigo-50 hover:text-indigo-600 text-xs font-medium text-slate-700 transition-colors flex items-center gap-1"
                  >
                    <span>{f.emoji}</span>
                    <span>{f.label}</span>
                  </button>
            ...
            {showEditLocationInput && (
              <div className="flex items-center gap-2 p-1.5 rounded-xl bg-white border border-slate-200 animate-in fade-in">
                <MapPin className="w-3.5 h-3.5 text-slate-400 ml-1.5" />
                <input
                  type="text"
                  value={editPostLocation || ''}
                  onChange={(e) => setEditPostLocation(e.target.value)}
                  placeholder="Enter location (e.g. Islamabad, Pakistan)..."
                  className="flex-1 text-xs text-slate-800 focus:outline-none"
                />
                <button
                  type="button"
                  onClick={() => setShowEditLocationInput(false)}
                  className="px-2 py-0.5 rounded text-[10px] font-semibold text-slate-500 hover:bg-slate-100"
                >
                  Done
                </button>
              </div>
            ...
            {/* Tools Bar & Save/Cancel */}
            <div className="flex items-center justify-between pt-1">
              <div className="flex items-center gap-1">
                <button
                  type="button"
                  onClick={() => {
                    setShowEditFeelingPicker((prev) => !prev);
                    setShowEditLocationInput(false);
                  }}
                  className="p-1.5 rounded-lg text-slate-500 hover:text-amber-600 hover:bg-white text-xs flex items-center gap-1 font-medium transition-colors"
                ...
                <button
                  type="button"
                  onClick={() => {
                    setShowEditLocationInput((prev) => !prev);
                    setShowEditFeelingPicker(false);
                  }}
                  className="p-1.5 rounded-lg text-slate-500 hover:text-emerald-600 hover:bg-white text-xs flex items-center gap-1 font-medium transition-colors"
                  title="Add Location"
                >
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  type="submit"
                  disabled={!editPostContent.trim() || isSavingPostEdit}
                  className="px-4 py-1.5 rounded-xl text-xs font-bold text-white bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 transition-colors shadow-xs flex items-center gap-1.5"
                >
                  {isSavingPostEdit ? (
                    <>
                      <Loader2 className="w-3.5 h-3.5 animate-spin" />
